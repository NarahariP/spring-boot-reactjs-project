...
public class ProjectController {
		...
		try {
			return ResponseEntity
					.created(ServletUriComponentsBuilder.fromCurrentRequest().path("{id}")
							.buildAndExpand(projectService.saveOrUpdateProject(
									ProjectUtils.projectFunction.apply(project), principal.getName()).getId())
							.toUri())
					.build();
	...
	public ResponseEntity<Project> retriveProject(@PathVariable String projectIdentifier, Principal principal) {
		return new ResponseEntity<>(projectService.retriveProject(projectIdentifier, principal.getName()),
				HttpStatus.OK);
	}
	...
	public ResponseEntity<?> deleteProject(@PathVariable String projectIdentifier, Principal principal) {
		projectService.delete(projectService.retriveProject(projectIdentifier, principal.getName()).getId());
		return new ResponseEntity<>(ProjectUtils.deleteFunction.apply(projectIdentifier), HttpStatus.OK);
	}

	@GetMapping("/projects")
	public List<Project> getAllProjects(Principal principal) {
		return projectService.retriveAllProjects(principal.getName());
	}
}
